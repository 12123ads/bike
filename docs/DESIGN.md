...
| Air780EP PWRKEY | **P1.00** | J2.10 | 开集拉低 >1 s，内部已有 5.6 k，**不要外加上拉** |
...
**合计 13 个引脚**（8 个信号 + 一对 UART ×2 + 一对 I2C 各算两个）。
**ADR-001 里那个 `LM5164 PGOOD` 引脚已删除** —— 换成 LX-P160 成品模块后不存在。
**Air780EP RI 那一行也删除了**：脚号至今无来源（§11 #18），主控侧从来没有
代码监听它，而 overlay 里原来填的 P0.26 恰好是个没引出的脚 —— 一个指向
虚空的 DT 节点比没有节点更糟。查到 `Air780EP硬件手册V1.1.pdf` 之后
连监听代码一起加，脚号从下面的余量里取。

**余量 8 个**：P0.02(AIN0) P0.09 P0.29(AIN5) P1.01 P1.02 P1.04 P1.06 P1.07。
**BLE 一个都不占**（片上无线 + 板载天线，§3.3）。锁要 3 根线（驱动 + 反馈 +
使能）也装得下，INT2 那根潜在的第二中断线（§3.7）同样有地方接 ——
这些余量在 NFC 方案下是没有的（那时零余量）。
...
| 模组 → nRF | **MAIN_RI**，需 `AT+CFGRI=1` → 出 120 ms 低脉冲 | +1 GPIO（脚号未定，DT 节点已删，§3.5） |
...
| 18 | **MAIN_DTR / MAIN_RI 具体脚号无来源 `[未核实]`**（§8.7） | **布板前硬门禁**，必须查硬件手册 V1.1 | 仍无来源。2026-09-04 **删掉了 overlay 里的 `modem_ri` 节点** —— 原来填的 P0.26 是这块板没引出的脚（§3.5），而且从来没有 C 代码取用它。定脚号时从 §3.5 的 8 个余量里取，连监听代码一起加 |
